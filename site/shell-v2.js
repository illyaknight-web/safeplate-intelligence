...
function patchPublicRecallModal(frame){
  ...
  try{
    ...
    if(d.documentElement.dataset.safeplateRecallFreshnessPatch==='7'){scheduleAutoOpen();return}
    d.documentElement.dataset.safeplateRecallFreshnessPatch='7';
    ...
    const foodName=x=>{
      ...
      const company=[x?.company,x?.firm_name,x?.recalling_firm,x?.brand].map(v=>String(v||'').trim()).find(Boolean);
      if(food&&company&&food.toLowerCase().startsWith(company.toLowerCase()))food=food.slice(company.length).replace(/^[\s:—-]+/,'').trim();
      food=food.replace(/^(?:a|an|the)\s+/i,'').replace(/\brecall notice\b.*$/i,'').replace(/[\s,;:.—-]+$/,'').trim();
