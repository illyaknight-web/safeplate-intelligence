...
function patchPublicRecalls(){
 try{
  ...
  const render=()=>{box.innerHTML=CURRENT_RECALLS.map(r=>`<article class="alertItem"><img src="${r.image}" alt="${r.alt}"><div><small>Recall date · ${r.date}</small><b>${r.title}</b><div style="margin-top:6px;color:#5a665d;font-size:11px;line-height:1.35">${r.reason}</div><a href="${r.url}" target="_blank" rel="noopener">Verify at official source</a></div></article>`).join('')};
  render();
  if(!dialog.dataset.safeplateCurrentFive){dialog.dataset.safeplateCurrentFive='1';new MutationObserver(()=>{if(!dialog.hidden)render()}).observe(dialog,{attributes:true,attributeFilter:['hidden']})}
