...
async function kickStateScan(req, stateScanLastSync, scannerVersion){
  ...
  if(age<=35 && scannerVersion==="2.1")return false;
...
export default async(req)=>{
 ...
 const stateScanDispatched=await kickStateScan(req,stateScanLastSync,coverage.scannerVersion||s.meta?.stateScanVersion||null);
 return Response.json({
   ...
   stateSurveillance:{live:stateScanFresh&&coverage.checked===51&&coverage.scannerVersion==="2.1",scannerVersion:coverage.scannerVersion||null,lastSync:stateScanLastSync,ageMinutes:stateScanAgeMinutes,cycleMinutes:30,jurisdictionsTotal:51,jurisdictionsChecked:coverage.checked||0,jurisdictionsOnline:coverage.online||0,jurisdictionIssues:coverage.degraded||0,stateSignals:coverage.signals||0,healthEntries:stateSources.length}
