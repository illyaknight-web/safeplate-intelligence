...
export function runBacktest(corpus={}){
  ...
  return {protocolVersion:'SAFEPLATE-BACKTEST-1.0',generatedAt:new Date().toISOString(),caseCount:cases.length,validCaseCount:valid.length,insufficientDataCaseCount:cases.filter(x=>x.outcome==='INSUFFICIENT_DATA').length,confusionMatrix:{truePositive:tp,falsePositive:fp,falseNegative:fn,trueNegative:tn},metrics:{sensitivity,precision,falsePositiveRate,meanLeadTimeHours:leads.length?+(leads.reduce((a,b)=>a+b,0)/leads.length).toFixed(2):null,measurableLeadTimeCases:leads.length},metricsStatus:adequate?'VALIDATION_THRESHOLD_MET':'PROVISIONAL_NOT_FOR_PERFORMANCE_CLAIMS',metricsWarning:adequate?null:'Do not cite sensitivity, precision, false-positive rate, or average lead time as system performance until the minimum validation evidence is met.',releaseGate:adequate?'ELIGIBLE_FOR_HUMAN_REVIEW':'HOLD_INSUFFICIENT_VALIDATION',minimumEvidence:{historicalCases:10,negativeControls:3,measurableLeadTimeCases:3},cases};
}
