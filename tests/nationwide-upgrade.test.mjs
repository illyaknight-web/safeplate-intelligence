...
test('backtest reports lead time but holds release with inadequate controls',()=>{const r=runBacktest({cases:[{id:'case',status:'CONFIRMED',ground_truth_positive:true,earliest_credible_signal_at:'2026-07-01',recall_published_at:'2026-07-03'}]});assert.equal(r.metrics.meanLeadTimeHours,48);assert.equal(r.releaseGate,'HOLD_INSUFFICIENT_VALIDATION');assert.equal(r.confusionMatrix.truePositive,1)});
